function [colmat,dotmat,dotsize,rdkidx,frames,varargout] = RDK_init_FShift_HighFreqFlick(scr,Propixx,RDK,trial,cross)
...
%                   flicker_type    'SSVEP'; 'BRBF';'SSVEP_inlet' SSVEP vs broad band flicker; default: SSVEP
...
% v20250916:
%       - adapted version that uses potential inlet flicker
%
% To do:
...
for r = 1:length(RDK.RDK)
    ...
    switch RDK.RDK(r).flicker_type
        ...
        case 'SSVEP_inlet'
            ...
            % define on frames of inlet
            onframes = scr.refrate/RDK.RDK(r).in_freq/2; %number of "on"-frames
            ...
            % find number of decimals for frequency to create vector that ends with full cycle (for frequencies with decimal point)
            if mod(RDK.RDK(r).in_freq,1) > 0
                t.temp = strsplit(num2str(RDK.RDK(r).in_freq),'.');
                t.dec = length(t.temp{2});
            else
                t.dec = 0;
            end
            ...
            t.frames = 0:t.framenum-1;
            % create raw weights with interpolated flanks (is longer than it should be
            weight_raw = MakeFlicker(t.frames,scr.refrate/RDK.RDK(r).in_freq);
            % figure; plot(t.frames./scr.refrate, weight_raw)
            % prune weights according to number of pre-cue and post-cue frames
            t.idx = [ (1:fr_cue-1) + numel(weight_raw)/2 - (fr_cue-1)... % pre-cue
                (numel(weight_raw)/2)+(1:frames.pertrial-fr_cue+1)]; % post-cue
            weight_i = weight_raw(t.idx); % weight for inlets
            weight = [ones(size(weight_i)); weight_i];% weight for ring + inlet
            % figure; plot(t.frames(t.idx)./scr.refrate, weight)
            % figure; plot( weight)
...
%             xlim([0 100])
    ...
    % index which dots are moved coherently | if there are two parts, they need to be moved in sync
    t.cohidx = randsample(RDK.RDK(r).num,ceil(RDK.RDK(r).num*RDK.event.coherence));
    t.cohidx2 = false(1,size(colmat_r,2)); 
    for i_part = 1:numel(unique(partidx))
        t.idx = find(partidx==i_part);
        t.cohidx2(t.idx(t.cohidx))=true;
    end
    for w=1:length(weight)
        for i_part = 1:numel(unique(partidx))
            if  strcmp(RDK.event.type, 'colorchange') & ... % is color change the event class?
                    any((w >= trial.event.onset & w <= (trial.event.onset+RDK.event.duration*scr.refrate)) & ... % time of event?
                    RDK.RDK(r).id == trial.event.RDK) & ...                    % current RDK is event RDK
                    i_part == 1;
...
%     alphamat(alphamat>=0)= 1; % set all transparency values to 1;
    ...
    dotmat_pr = zeros(2,  RDK.RDK(r).num, frames.pertrial); %setup dot matrix: xy x dot number x frames
    ...
    if Propixx == 4
        ...
        dotmat_pr(:,:,shift_idx) = [dotmat_pr(1,:,shift_idx)+scr.shift(p,1); dotmat_pr(2,:,shift_idx)+scr.shift(p,2)];
        end
    end   
    ...
    rdkidx_r(1:RDK.RDK(r).num*numel(unique(partidx)),1:frames.pertrial) = RDK.RDK(r).id;
    
    % define size for each dot
    if numel(unique(partidx))>1
        dotsize_r = repmat([RDK.RDK(r).dot_size; RDK.RDK(r).in_dot_size],[RDK.RDK(r).num frames.pertrial]);
    else
        dotsize_r = repmat(RDK.RDK(r).dot_size,size(rdkidx_r));
    end

    % append partidx
    partidx_r = repmat(partidx', [1 frames.pertrial]);
    ...
    partidxmat = cat(1,partidxmat,partidx_r);
...
if numel(unique(partidxmat))>1
    t.idx = Shuffle(find(partidxmat(:,1)==1));
    t.idx2=[t.idx';t.idx'+1];
    shuffle_idx=t.idx2(:);
else
    shuffle_idx = Shuffle(find(partidxmat(:,1)==1));
end
rdkidx = rdkidx(shuffle_idx,:); %ToDo: weg?
colmat = colmat(:,shuffle_idx,:);
dotmat = dotmat(:,shuffle_idx,:);
dotsize = dotsize(shuffle_idx,:);
partidxmat = partidxmat(shuffle_idx,:);
