function [] = run_FShift_HighFreqFlick_v2(sub,flag_training, flag_isolum, flag_block)
% run_FShift_HighFreqFlick(sub,flag_training, flag_isolum, flag_block) also known as HiFli
%   runs experiment SSVEP_FShift_HighFreqFlick
...
p.stim.RDKcenter        = [1 2; 1 2; ... % defines which RDK are shown in center [1 2 = convent. RDKs; 5 6 = inlet colored flicker large; ; 9 10 = convent. RDKs flicker high freq]
                            5 6; 5 6; ...
                            9 10; 9 10]; 
p.stim.RDKperi          =  [3 4; 3 4; ...   % defines which RDK are shown in the periphery [3 4 = convent. RDKs; 7 8 = inlet colored flicker; 11 12 = convent. RDKs]
                            7 8; 7 8; ...
                            11 12; 11 12];
...
RDK.RDK(1).in_dot_size  = 10;                           % size of dots sames as outside; used to be 6 in old study
...
% logfiles
p.log.path              = '/home/stimulation120/matlab/user/christopher/stim_FShift_HighFli/logfiles_v2/';
...
% for debugging: ps.xCenter = round(p.scr_res(1)/2); ps.yCenter = round(p.scr_res(2)/2)
% fixation cross
...
% keep position fixed across participants?
t.pos = repmat(p.stim.pos_shift(1,:),100,1);
...
% randomize frequencies for conventional high freq RDKs
t.val = num2cell(p.stim.in_freqs{1}(randperm(2)));
[RDK.RDK(9:10).freq] = t.val{:};
t.val = num2cell(p.stim.in_freqs{2}(randperm(2)));
[RDK.RDK(11:12).freq] = t.val{:};
...
rng(p.sub,'v4')
randmat.experiment = rand_FShift_HighFreqFlick(p, RDK,  0);    % randomization
for i_bl = p.flag_block:p.stim.blocknum
    % start experiment
    [timing.experiment{i_bl},button_presses.experiment{i_bl},resp.experiment{i_bl}] = ...
        pres_FShift_HighFreqFlick(p, ps, key, RDK, randmat.experiment, i_bl,0);
    % save logfiles
    save(sprintf('%s%s',p.log.path,p.filename),'timing','button_presses','resp','randmat','p', 'RDK')
